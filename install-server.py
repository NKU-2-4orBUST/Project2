...
def nfs(): # Function to Install NFS
	...
	os.system('echo Installing NSF and Downloading NFS Configuration files.... | tee  -a /var/log/ldap_install.log; yum install -y nfs-utils rpcbin >> /var/log/nfs _install.log 2>&1; wget https://github.com/NKU-2-4orBUST/Project2/raw/main/exports.txt  >> /var/log/nfs _install.log 2>&1; exportfs -a  >> /var/log/nfs _install.log 2>&1')
	# Exporting /HOME directory and starting NFS and required services
	os.system('echo Exporting /HOME directory and starting NFS and required services.... | tee  -a /var/log/ldap_install.log; cat exports.txt > /etc/exports  >> /var/log/nfs _install.log 2>&1; systemctl start nfs  >> /var/log/nfs _install.log 2>&1; systemctl start nfslock >> /var/log/nfs _install.log 2>&1; systemctl enable slapd >> /var/log/nfs_install.log 2>&1; systemctl status nfs nfslock  >> /var/log/nfs _install.log 2>&1') 
	# Firewall configured and reloaded.
	os.system('echo Configuring Firewall for NFS.... | tee  -a /var/log/ldap_install.log; firewall-cmd --zone=public --add-port=2049/tcp --permanent  >> /var/log/nfs _install.log 2>&1; firewall-cmd --zone=public --add-port=111/tcp --permanent >> /var/log/nfs _install.log 2>&1; firewall-cmd --zone=public --add-port=20048/tcp --permanent >> /var/log/nfs _install.log 2>&1; firewall-cmd --zone=public --add-port=2049/udp --permanent >> /var/log/nfs _install.log 2>&1; firewall-cmd --zone=public --add-port=111/udp --permanent >> /var/log/nfs _install.log 2>&1; firewall-cmd --zone=public --add-port=20048/udp --permanent >> /var/log/nfs _install.log 2>&1; echo Restarting Firewall.... | tee  -a /var/log/ldap_install.log; systemctl restart firewalld >> /var/log/nfs _install.log 2>&1; sleep 5; firewall-cmd --list-all >> /var/log/nfs _install.log 2>&1')
