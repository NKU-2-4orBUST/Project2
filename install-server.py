...
def ldap():
	...
	os.system('echo Configuring LDAP.... | tee  -a /var/log/ldap_install.log; cat /tmp/olc2 > /etc/openldap/slapd.d/cn\=config/olcDatabase\=\{2\}hdb.ldif; cp /usr/share/openldap-servers/DB_CONFIG.example /var/lib/ldap/DB_CONFIG; cat ./migrate_common.ph > /usr/share/migrationtools/migrate_common.ph; cat ./base.ldif > /usr/share/migrationtools/base.ldif; tar -xvf diradm-1.3.tar.gz -C /usr/local; cp /usr/local/diradm-1.3/diradm.conf /etc/; cat ./diradm.conf > /etc/diradm.conf')
	# Migrates system user and groups to LDAP services and set the owner of all files in /var/lib/ldap as ldap
	os.system('echo Migrating Users and Groups to LDAP database.... | tee  -a /var/log/ldap_install.log; systemctl start slapd  >> /var/log/ldap_install.log 2>&1; ldapadd -Y EXTERNAL -H ldapi:// -f /etc/openldap/schema/core.ldif >> /var/log/ldap_install.log 2>&1; ldapadd -Y EXTERNAL -H ldapi:// -f /etc/openldap/schema/cosine.ldif >> /var/log/ldap_install.log 2>&1; ldapadd -Y EXTERNAL -H ldapi:// -f /etc/openldap/schema/nis.ldif >> /var/log/ldap_install.log 2>&1; ldapadd -Y EXTERNAL -H ldapi:// -f /etc/openldap/schema/inetorgperson.ldif >> /var/log/ldap_install.log 2>&1; systemctl stop slapd >> /var/log/ldap_install.log 2>&1; chown -R ldap:ldap /var/lib/ldap >> /var/log/ldap_install.log 2>&1; slapadd -v -l /usr/share/migrationtools/base.ldif >> /var/log/ldap_install.log 2>&1; /usr/share/migrationtools/migrate_passwd.pl /etc/passwd > /usr/share/migrationtools/passwd.ldif >> /var/log/ldap_install.log 2>&1; slapadd -v -l /usr/share/migrationtools/passwd.ldif >> /var/log/ldap_install.log 2>&1; /usr/share/migrationtools/migrate_group.pl /etc/group > /usr/share/migrationtools/group.ldif >> /var/log/ldap_install.log 2>&1; slapadd -v -l /usr/share/migrationtools/group.ldif >> /var/log/ldap_install.log 2>&1; chown -R ldap:ldap /var/lib/ldap >> /var/log/ldap_install.log 2>&1; systemctl start slapd >> /var/log/ldap_install.log 2>&1; systemctl -l status slapd >> /var/log/ldap_install.log 2>&1')
	# Configures Firewall for LDAP service
	os.system('echoConfiguring firewall for LDAP services.... | tee  -a /var/log/ldap_install.log; firewall-cmd --zone=public --add-port=389/tcp --permanent >> /var/log/ldap_install.log 2>&1; firewall-cmd --zone=public --add-port=636/tcp --permanent >> /var/log/ldap_install.log 2>&1; firewall-cmd --reload >> /var/log/ldap_install.log 2>&1')
def nfs():
