...
def ldap():
	...
	os.system('echo Configuring LDAP.... | tee  -a /var/log/ldap_install.log; cat /tmp/olc2 > /etc/openldap/slapd.d/cn\=config/olcDatabase\=\{2\}hdb.ldif; rm /var/lib/ldap/* >> /var/log/ldap_install.log 2>&1; cp /usr/share/openldap-servers/DB_CONFIG.example /var/lib/ldap/DB_CONFIG; cat ./migrate_common.ph > /usr/share/migrationtools/migrate_common.ph >> /var/log/ldap_install.log 2>&1; cat ./base.ldif > /usr/share/migrationtools/base.ldif >> /var/log/ldap_install.log 2>&1; tar -xvf diradm-1.3.tar.gz -C /usr/local >> /var/log/ldap_install.log 2>&1; cp /usr/local/diradm-1.3/diradm.conf /etc/ >> /var/log/ldap_install.log 2>&1; cat ./diradm.conf > /etc/diradm.conf >> /var/log/ldap_install.log 2>&1')
